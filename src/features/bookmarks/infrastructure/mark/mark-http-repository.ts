...
export class MarkHttpRepository implements MarkRepository {
  ...
  async findAll(): Promise<any> {
    ...
    if (foldersContent.folders.length !== 0) {
      const folders: Folder[] = foldersContent.folders.map((folder: FolderDto) => {
  ...
  async create(createMarkDto: CreateMarkDto, idFolder: string): Promise<any> {
    return await http
      .post('/mark', { createMarkDto: createMarkDto, folderId: idFolder })
      .then(() => {
        return null
      })
      .catch(error => {
  ...
  async edit(mark: Mark): Promise<any> {
    return await http
      .put(`/mark/${mark.id}`, { editMarkDto: this.markToEditMarkDtoMapper.map(mark) })
      .then(() => {
        return null
      })
      .catch(error => {
  ...
  async createFolder(createFolderDto: CreateFolderDto): Promise<any> {
    ...
    return await http
      .post('/folder', { createFolderDto: createFolderDto })
      .then(() => {
        return null
      })
      .catch(error => {
  ...
  async editFolder(folder: Folder): Promise<any> {
    return await http
      .put(`/folder/${folder.id}`, { editFolderDto: this.folderToEditFolderDtoMapper.map(folder) })
      .then(() => {
        return null
      })
      .catch(error => {
